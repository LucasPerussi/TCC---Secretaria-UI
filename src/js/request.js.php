...
<script>
    $("#closeTicket").submit(function(e) {
        e.preventDefault();
        const data = new FormData(e.target);
        const object = Object.fromEntries(data.entries());
        axios.post('<?= Config::BASE_ACTION_URL ?>/close-ticket', object)
            .then(function(response) {
                console.log(response)
                if (response.data.status > 202) {
                    throw response.data;
...
<script>
    $("#changeStage").submit(function(e) {
        e.preventDefault();
        const data = new FormData(e.target);
        const object = Object.fromEntries(data.entries());
        axios.post('<?= Config::BASE_ACTION_URL ?>/change-stage', object)
            .then(function(response) {
                console.log(response)
                if (response.data.status > 202) {
                    throw response.data;
